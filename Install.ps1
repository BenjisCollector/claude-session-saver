...
# Save shortcut
$lnk = $WshShell.CreateShortcut("$startMenu\Save Claude Sessions.lnk")
$lnk.TargetPath = "powershell.exe"
$lnk.Arguments = "-ExecutionPolicy Bypass -WindowStyle Hidden -File `"$root\Save-Sessions.ps1`" -Silent"
...
# Restore shortcut
$lnk = $WshShell.CreateShortcut("$startMenu\Restore Claude Sessions.lnk")
$lnk.TargetPath = "powershell.exe"
$lnk.Arguments = "-ExecutionPolicy Bypass -File `"$root\Restore-Sessions.ps1`""
$lnk.WorkingDirectory = $root
...
$lnk = $WshShell.CreateShortcut("$startupDir\Claude Session Saver.lnk")
$lnk.TargetPath = "powershell.exe"
$lnk.Arguments = "-ExecutionPolicy Bypass -WindowStyle Hidden -File `"$root\SessionSaver.ps1`""
$lnk.WorkingDirectory = $root
$lnk.IconLocation = "shell32.dll,258"
...
Write-Host "    2. Right-click the tray icon > Save Sessions (before shutting down)" -ForegroundColor Gray
Write-Host "    3. Right-click the tray icon > Restore Sessions (after booting up)" -ForegroundColor Gray
