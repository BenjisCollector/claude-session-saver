<#
.SYNOPSIS
    Installs Cryosave: adds system tray app to Windows startup
...
Write-Host ''
Write-Host '  Cryosave - Install' -ForegroundColor Cyan
...
$lnk = $WshShell.CreateShortcut([System.IO.Path]::Combine($startMenu, 'Cryosave - Freeze.lnk'))
$lnk.TargetPath = 'powershell.exe'
$lnk.Arguments = ('-ExecutionPolicy Bypass -WindowStyle Hidden -File "{0}" -Silent' -f [System.IO.Path]::Combine($root, 'Freeze.ps1'))
$lnk.WorkingDirectory = $root
$lnk.IconLocation = 'shell32.dll,258'
$lnk.Description = 'Freeze all terminal windows and Claude Code sessions'
$lnk.Save()
Write-Host '  [OK] Start Menu: Cryosave - Freeze' -ForegroundColor Green

$lnk = $WshShell.CreateShortcut([System.IO.Path]::Combine($startMenu, 'Cryosave - Thaw.lnk'))
$lnk.TargetPath = 'powershell.exe'
$lnk.Arguments = ('-ExecutionPolicy Bypass -File "{0}"' -f [System.IO.Path]::Combine($root, 'Thaw.ps1'))
$lnk.WorkingDirectory = $root
$lnk.IconLocation = 'shell32.dll,137'
$lnk.Description = 'Thaw frozen terminal windows and Claude Code sessions'
$lnk.Save()
Write-Host '  [OK] Start Menu: Cryosave - Thaw' -ForegroundColor Green

# 2. Add tray app via Scheduled Task (faster than Startup folder)

$taskName = 'CryosaveTray'
$trayScript = [System.IO.Path]::Combine($root, 'Cryosave.ps1')

# Remove old Startup folder shortcut if it exists (migration from v1.0)
$oldStartupLnk = [System.IO.Path]::Combine($env:APPDATA, 'Microsoft\Windows\Start Menu\Programs\Startup\Cryosave.lnk')
...
Register-ScheduledTask `
    ...
    -Trigger $trigger `
    -Settings $settings `
    -Description 'Cryosave system tray app' `
